...
class ConfigObject:
    ...
    """
        ...
        for config_dict in combinations:
            ...
            warm_nodes = ConfigObject.enumerate_warm_nodes(
                num_warm_nodes, starting_server_node, 12, 13, 14, 19
            )
    ...
    """
        ...
        while start_ip_enum in excluded_ip_enums:
            start_ip_enum += 1
        regioned_nodes = [node.Node(start_ip_enum, region=0, store="/data")]

        second_enum = start_ip_enum + 1
        if num_warm_nodes >= 2:
            while second_enum in excluded_ip_enums:
                second_enum += 1
            regioned_nodes.append(node.Node(second_enum, region=1, store="/data"))

        third_enum = second_enum + 1
        if num_warm_nodes >= 3:
            while third_enum in excluded_ip_enums:
                third_enum += 1
            regioned_nodes.append(node.Node(third_enum, region=2, store="/data"))
        ...
        for i in range(remaining_num_warm_nodes):
            ...
            while (ip_enum + booster) in excluded_ip_enums:
                booster += 1
            ip_enum += booster
            n = node.Node(ip_enum, region=i+3, store="/data")
