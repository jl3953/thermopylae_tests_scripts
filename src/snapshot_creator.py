...
from src import populate_crdb_data, run_single_data_point
...
def initialize_crdb(config):
    ...
    server_urls =  server_urls = ["postgresql://root@{0}:26257?sslmode=disable"\
                                    .format(n["ip"])
...
def main():
    ...
    data_files = ["populate1B._{0}.csv.gz".format(i)
                  for i in range(args.start, args.last)]
    ...
    for i in range(args.start, args.last + 1, 50):
        ...
        if last_snapshot is not None:
            run_single_data_point.restore_rows(server_node, last_snapshot)
        ...
        for j in range(first_file + 10, last_file+1, 10):
            ...
            populate_crdb_data.import_into_crdb(server_node, data_files[
                                                             idx-10:idx])
        ...
        populate_crdb_data.snapshot(server_node, cur_snapshot)
